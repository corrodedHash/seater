...
async function setup_user_info() {
  let user_info;
  try {
    user_info = await fetch("/api/user")
      .then((v) => v.json())
      .catch(async () => {
  ...
  return {
    ...
    waiting_rooms: user_info.waiting_rooms,
  };
}
...
function App() {
  ...
  const waiting_rooms = useSelector(
    (state: RootState) => state.waiting_room.value
  );
  ...
  return (
    <>
      ...
      {waiting_rooms.map((v) => {
        return (
          <div className="waitingRoom" key={v}>
            {v}
          </div>
        );
      })}
